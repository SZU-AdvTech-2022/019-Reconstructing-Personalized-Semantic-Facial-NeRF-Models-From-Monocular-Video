...
class NeRFRenderer(nn.Module):
    def __init__(self,
        ...
        self.bound = bound
        self.cascade = 1 + max(math.ceil(math.log2(bound)), 0)
        self.grid_size = 128
    ...
    def run(self, rays_o, rays_d, bs_weight=None, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        # query SDF and RGB
        density_outputs = self.density(xyzs.reshape(-1, 3), bs_weight=bs_weight)
        ...
        if upsample_steps > 0:
            ...
            new_density_outputs = self.density(new_xyzs.reshape(-1, 3), bs_weight=bs_weight)
    ...
    def run_cuda(self, rays_o, rays_d, bs_weight=None, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.training:
            ...
            sigmas, rgbs = self(xyzs, dirs, bs_weight)
    ...
    def update_extra_state(self, max_weight, decay=0.95, S=128, bs_weight=None):
                ...
                for ys in Y:
                    for zs in Z:
                        ...
                        for cas in range(self.cascade):
                            ...
                            # query density
                            sigmas = self.density(cas_xyzs, bs_weight=bs_weight)['sigma'].reshape(-1).detach()
        ...
        # ema update
        valid_mask = (self.density_grid >= 0) & (tmp_grid >= 0) # 这里取并集，上一次有值的地方和本次有值的地方取交集
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096, weight=None, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: pred_rgb: [B, N, 3]

        if self.cuda_ray:
            _run = self.run_cuda # 在run中使用raymarching
        else:
            _run = self.run
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    results_ = _run(rays_o[b:b+1, head:tail], rays_d[b:b+1, head:tail], bs_weight=weight, **kwargs)
            ...
            results = _run(rays_o, rays_d, bs_weight=weight, **kwargs)
