...
def get_rays(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1, rect=None):
    ...
    if N > 0:
        ...
        if patch_size > 1:
            ...
            if rect is not None:
                ...
                for ii in range(num_patch):
                    ...
                    if rand_num < 0.5:
                        i_x = torch.randint(xmin, min(xmax, H-patch_size), size=[1], device=device)
                        i_y = torch.randint(ymin, min(ymax, W-patch_size), size=[1], device=device)
                        inds[ii,:] = torch.tensor([i_x, i_y], device=device) # 仅修改
        ...
        inds = torch.arange(H*W, device=device).expand([B, H*W]) # N=-1当测试的时候，光线就等于H*W
...
class PSNRMeter:
    ...
    def update(self, preds, truths):
        ...
        # np.save("debug1pred.npy", preds)
        # np.save("debug2gt.npy", truths)
        # print("Debug PSNR::", psnr)
        self.V += psnr
        self.N += 1
...
class SSIMMeter:
    ...
    def prepare_inputs(self, *inputs):
        ...
        for i, inp in enumerate(inputs):
            if torch.is_tensor(inp):
                inp = inp.detach().cpu().numpy().squeeze() # compare_ssim win_size or batchsize set to one
            outputs.append(inp)

        return outputs

    def update(self, preds, truths):
        preds, truths = self.prepare_inputs(preds, truths) # [H, W, 3], range[0, 1]
        ssim = compare_ssim(preds, truths, multichannel=True)
        # psnr = -10 * np.log10(np.mean((preds - truths) ** 2))
        self.V += ssim
        self.N += 1
...
class L1Meter:
    ...
    def update(self, preds, truths):
        preds, truths = self.prepare_inputs(preds, truths) # [B, N, 3] or [B, H, W, 3], range[0, 1]
        mse = np.mean(np.abs(preds - truths))
        self.V += mse
        self.N += 1
    ...
    def write(self, writer, global_step, prefix=""):
        writer.add_scalar(os.path.join(prefix, "L1"), self.measure(), global_step)

    def report(self):
        return f'L1 = {self.measure():.6f}'
...
class Trainer(object):
    def __init__(self, 
        ...
        self.vggloss = VGGLoss(device=self.device)
    ...
    def train_step(self, data):
        ...
        if 'images' not in data:
            ...
            outputs = self.model.render(rays_o, rays_d, weight=weight, staged=False, bg_color=None, perturb=True, force_all_rays=True, **vars(self.opt))
        ...
        outputs = self.model.render(rays_o, rays_d, weight=weight, staged=False, bg_color=bg_color, perturb=True, force_all_rays=False if self.opt.patch_size == 1 else True, **vars(self.opt))
        ...
        loss = self.model.lambda1 * self.criterion(pred_rgb, gt_rgb).mean(-1) # [B, N, 3] --> [B, N]
        # import pdb; pdb.set_trace()

        pred_rgb_mask = outputs['weights_sum']
        # print(pred_rgb_mask)
        loss += self.model.lambda2 * self.criterion(pred_rgb_mask.flatten(), gt_mask.flatten()).mean(-1)
        # print(pred_rgb_mask.flatten())
        ...
        if self.opt.patch_size > 1:
            ...
            loss = loss + self.model.lambda3 * self.criterion_lpips(pred_rgb, gt_rgb)
            # loss = loss + self.model.lambda3 * self.vggloss(pred_rgb, gt_rgb)
        ...
        # loss = loss + self.model.lambda3 * loss_ws.mean()

        # with torch.no_grad():
        #     with torch.cuda.amp.autocast(enabled=self.fp16):
        #         preds, preds_depth, truths, loss = self.eval_step(data)
        #     my_vis = preds[0].detach().cpu().numpy()
        #     my_vis = (my_vis * 255).astype(np.uint8)
        #     impath = os.path.join(self.workspace, 'vis', str(self.local_step)+'.png')
        #     cv2.imwrite(impath, cv2.cvtColor(my_vis, cv2.COLOR_RGB2BGR))
    ...
    def eval_step(self, data):
        ...
        outputs = self.model.render(rays_o, rays_d, weight=weight, staged=True, bg_color=bg_color, perturb=False, **vars(self.opt))
    ...
    def test_step(self, data, bg_color=None, perturb=False):  
        ...
        outputs = self.model.render(rays_o, rays_d, weight=weight, staged=True, bg_color=bg_color, perturb=perturb, **vars(self.opt))
    ...
    def train(self, train_loader, valid_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            self.epoch = epoch
            if self.opt.patch_size <= 1: # so ugly
                if epoch <= 2:
                    self.model.lambda1, self.model.lambda2, self.model.lambda3 = 1, 1, 0
                elif epoch <= 7:
                    self.model.lambda1, self.model.lambda2, self.model.lambda3 = 1, 0, 0
                else:
                    self.opt.patch_size = 32
                    self.model.lambda1, self.model.lambda2, self.model.lambda3 = 0.1, 0, 0.01
                    # self.model.lambda1, self.model.lambda2, self.model.lambda3 = 0.1, 0, 0
            else:
                if epoch <= 2:
                    self.model.lambda1, self.model.lambda2, self.model.lambda3 = 1, 1, 0
                elif epoch <= 7:
    ...
    def train_gui(self, train_loader, step=16):
        ...
        for _ in range(step):
            ...
            if self.model.cuda_ray and self.global_step % self.opt.update_extra_interval == 0:
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    self.model.update_extra_state(data['max_weight'])
    ...
    def test_gui(self, pose, intrinsics, W, H, bg_color=None, spp=1, downscale=1):
        ...
        data = {
            ...
            'H': rH,
            'W': rW,
            # 'weight': torch.zeros((rays['rays_o'].shape[0], rays['rays_o'].shape[1], 46)) # 丑陋的实现
            'weight' : None,
        }
        # print(data['rays_o'].shape)
