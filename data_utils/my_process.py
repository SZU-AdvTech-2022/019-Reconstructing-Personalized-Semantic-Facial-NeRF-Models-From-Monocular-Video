...
if __name__ == '__main__':
    ...
    coeffs_dir = os.path.join(base_dir, 'coeffs')
    ...
    #     extract_audio(opt.path, wav_path) # 不需要

    # # extract audio features
    # if opt.task == -1 or opt.task == 2:
    #     extract_audio_features(wav_path, mode=opt.asr) # 不需要
    ...
    if opt.task == -1 or opt.task == 10:
        os.system("python " + workspace_dir + "/CPEM/test.py" + \
            " --mode get_coeffs --image_path " + ori_imgs_dir + \
            " --save_path " + coeffs_dir + " --checkpoint_path " +\
            workspace_dir + "/CPEM/data/pretrained_model/resnet50-id-exp-300000.ckpt")

    # save the max coeff
    if opt.task == -1 or opt.task == 11:
        from scipy.io import loadmat
        import numpy as np
        coeff_list = os.listdir(coeffs_dir)
        all_coeff = np.zeros((len(coeff_list), 46)) # 46 dim
        for name in coeff_list:
            idx = int(name[:-10]) # '{number}_coeff.mat'
            bs = loadmat(os.path.join(coeffs_dir,name))['coeff_bs'] # type: np.ndarray
            all_coeff[idx, :] = bs            
        np.save(os.path.join(base_dir, "max_coeffs.npy"), all_coeff.max(axis=0))
        np.save(os.path.join(base_dir, "min_coeffs.npy"), all_coeff.min(axis=0))


    if opt.task == -1 or opt.task == 12:
        import shutil
        val_path = os.path.join(base_dir, 'transforms_' + 'val' + '.json')
        test_path = os.path.join(base_dir, 'transforms_' + 'test' + '.json')
        shutil.copyfile(val_path, test_path)
'''
    if opt.task == 13:
        # 预处理绿幕人脸
        basename = os.path.basename(opt.path)[:-4]
        if basename != '0001' and basename != '0002':
            print(basename, "is not 0001 or 0002")
            exit()
        print("Start parsing...")
        image_list = os.listdir(gt_imgs_dir)
        # import pdb; pdb.set_trace()
        for imgpath in image_list:
            img = cv2.imread(os.path.join(gt_imgs_dir, imgpath))
            parse = cv2.imread(os.path.join(parsing_dir, imgpath[:-4] + '.png'))
            head_mask = cv2.inRange(parse, np.array([250,0,0]), np.array([255,0,0]))
            mask = cv2.inRange(img, (0, 100, 0), (90, 255, 90))
            cv2.bitwise_and(head_mask, mask, mask)
            # cv2.imwrite("pre_debug.png", mask)
            mask_inv = cv2.bitwise_not(mask)
            face_m = cv2.bitwise_and(head_mask, mask_inv)
            debug_face = cv2.bitwise_and(img, img, mask=face_m)
            index = np.where(mask == 255)
            parse[index[0], index[1], :] = np.array([255,255,255]) # parse[mask]不对，而且速度卡顿
            # cv2.imwrite("debug_parse.png", debug_face)
            cv2.imwrite(os.path.join(parsing_dir, imgpath), parse)
            print(imgpath, " parse done!")
        print("Done")
'''        
